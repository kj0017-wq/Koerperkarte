...
const currentPath = 'output/import/current-muscles.json';
...
function titleFromId(id) { return id.split('-').map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(' '); }
function parseList(value) { return (value || '').split(/[|,;]/).map((item) => item.trim()).filter(Boolean); }
function unique(values) { return Array.from(new Set(values.filter(Boolean))).sort((a, b) => a.localeCompare(b, 'de')); }
...
function inferBodyArea(muscleId, painRegions) {
  ...
  if (faceMuscles.some((item) => joined.includes(item))) return 'Kopf / Gesicht';
  ...
  if (painRegions.some((item) => ['head', 'face', 'jaw', 'tmj', 'ear', 'teeth', 'eye', 'orbit', 'temple', 'forehead'].includes(item))) return 'Kopf / Gesicht';
  return 'Koerper';
}
function inferMapType(muscleId, muscle, row) {
  ...
  if (anatomical.includes('kopf') || anatomical.includes('gesicht')) return 'face';
...
const muscles = JSON.parse((fs.readFileSync(currentPath, 'utf8') || '{}').replace(/^\uFEFF/, '').replace(/\u0000/g, ''));
...
let addedMuscles = 0, addedPoints = 0, updatedPoints = 0, skippedRows = 0;
const touched = new Set();
const newMuscles = [];
for (const row of rows) {
  const muscleId = row.muscleId;
  if (!muscleId || !row.triggerpointId) { skippedRows++; continue; }
  ...
  if (existingIndex >= 0) { muscle.triggerpoints[existingIndex] = { ...muscle.triggerpoints[existingIndex], ...point }; updatedPoints++; }
  else { muscle.triggerpoints.push(point); addedPoints++; }
  touched.add(muscleId);
}
for (const muscle of Object.values(muscles)) {
  if (Array.isArray(muscle.triggerpoints)) muscle.triggerpoints.sort((a, b) => String(a.id).localeCompare(String(b.id), 'de'));
}
const stamp = new Date().toISOString().replace(/[-:T.Z]/g, '').slice(0, 14);
const outJson = `${outDir}/muscles-merged-${stamp}.json`;
...
const reportRows = [
  ['metric', 'value'], ['csv_rows', rows.length], ['muscles_before', beforeMuscles], ['triggerpoints_before', beforePoints],
  ['muscles_after', afterMuscles], ['triggerpoints_after', afterPoints], ['new_muscles', addedMuscles], ['new_triggerpoints', addedPoints],
  ['updated_triggerpoints', updatedPoints], ['skipped_rows', skippedRows], ['touched_muscles', touched.size], ['new_muscle_ids', newMuscles.join('|')], ['json_path', outJson]
];
const reportPath = `${outDir}/triggerpoints-import-report-${stamp}.csv`;
fs.writeFileSync(reportPath, reportRows.map((r) => r.map((v) => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n'), 'utf8');
console.log(JSON.stringify({ csvRows: rows.length, beforeMuscles, beforePoints, afterMuscles, afterPoints, addedMuscles, addedPoints, updatedPoints, skippedRows, touchedMuscles: touched.size, newMuscles, outJson, reportPath }, null, 2));
